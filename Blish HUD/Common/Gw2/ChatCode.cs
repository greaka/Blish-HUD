...
namespace Blish_HUD.Common.Gw2
{
    public static class ChatCode
    {
        ...
        /// </summary>
        /// <param name="fullcode">
        ///     This needs to be the item code or chat code (i.e. from trading post or gw2spidy) not the
        ///     wardrobe ID
        /// </param>
        ...
        public static string GenerateChatCode(int itemId, int quantity = 1, int upgrade1Id = 0, int upgrade2Id = 0,
            int skinId = 0)
        {
            ...
            var ids = new[] {2, quantity % 256, itemId, separator, skinId, upgrade1Id, upgrade2Id};

            // Byte length for each part
            var lengths = new[] {1, 1, 3, 1, skinId > 0 ? 4 : 0, upgrade1Id > 0 ? 4 : 0, upgrade2Id > 0 ? 4 : 0};
