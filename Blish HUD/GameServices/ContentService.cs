...
namespace Blish_HUD
{
    public class ContentService : GameService
    {
        ...
        public BitmapFont DefaultFont12 => this._defaultFont12 ??
                                           (this._defaultFont12 = GetFont(FontFace.Menomonia, FontSize.Size12,
                                               FontStyle.Regular));

        public BitmapFont DefaultFont14 => this._defaultFont14 ??
                                           (this._defaultFont14 = GetFont(FontFace.Menomonia, FontSize.Size14,
                                               FontStyle.Regular));

        public BitmapFont DefaultFont16 => this._defaultFont16 ??
                                           (this._defaultFont16 = GetFont(FontFace.Menomonia, FontSize.Size16,
                                               FontStyle.Regular));

        public BitmapFont DefaultFont24 => this._defaultFont24 ??
                                           (this._defaultFont24 = GetFont(FontFace.Menomonia, FontSize.Size24,
                                               FontStyle.Regular));

        public BitmapFont DefaultFont32 => this._defaultFont32 ??
                                           (this._defaultFont32 = GetFont(FontFace.Menomonia, FontSize.Size32,
                                               FontStyle.Regular));

        public ContentManager ContentManager => BlishHud.ActiveContentManager;
        ...
        public void PlaySoundEffectByName(string soundName)
        {
            if (!_loadedSoundEffects.ContainsKey(soundName))
                _loadedSoundEffects.TryAdd(soundName, BlishHud.ActiveContentManager.Load<SoundEffect>($"{soundName}"));
        ...
        private static Texture2D TextureFromFile(string filepath)
        {
            if (File.Exists(filepath))
            {
                ...
                {
                    return Texture2D.FromStream(BlishHud.ActiveGraphicsDeviceManager.GraphicsDevice, fileStream);
                }
            }
                ...
                using (var refArchive = new ZipArchive(refFs, ZipArchiveMode.Read))
                {
                    ...
                    if (refEntry != null)
                    {
                        using (var textureStream = refEntry.Open())
                        {
                            ...
                            return Texture2D.FromStream(BlishHud.ActiveGraphicsDeviceManager.GraphicsDevice,
                                textureCanSeek);
                        }
                    }
                    ...
                    if (File.Exists($@"ref\{filepath}"))
                        File.Copy($@"ref\{filepath}", $@"ref\to-include\{filepath}", true);
        ...
        {
            return BlishHud.ActiveContentManager.Load<TextureAtlas>(textureAtlasName);
        }

        public void PurgeTextureCache(string textureName)
        {
            _loadedTextures.TryRemove(textureName, out _);
        }
        ...
        /// <param name="size">
        ///     Specifies the size of the texture requested - only some render service hosts will utilize this
        ///     setting.
        /// </param>
        ...
        public AsyncTexture2D GetRenderServiceTexture(string signature, string fileId, RenderServiceTextureSize size)
        {
            ...
            var returnedTexture = new AsyncTexture2D(Textures.TransparentPixel.Duplicate());

            var requestUrl = $"https://darthmaim-cdn.de/gw2treasures/icons/{signature}/{fileId}-{(int) size}px.png";
            ...
            requestUrl.GetBytesAsync()
                .ContinueWith(textureDataResponse =>
                {
                    ...
                    {
                        Logger.Warn(textureDataResponse.Exception, "Request to render service for {textureUrl} failed.",
                            requestUrl);
                        return;
                    }
        ...
        {
            var fullFontName =
                $"{font.ToString().ToLowerInvariant()}-{((int) size).ToString()}-{style.ToString().ToLowerInvariant()}";

            if (!_loadedBitmapFonts.ContainsKey(fullFontName))
            {
                var loadedFont =
                    BlishHud.ActiveContentManager.Load<BitmapFont>(
                        $"fonts\\{font.ToString().ToLowerInvariant()}\\{fullFontName}");
        ...
        private static readonly Regex _regexRenderServiceSignatureFileIdPair = new Regex(@"(.{40})\/(\d+)(?>\..*)?$",
            RegexOptions.Singleline | RegexOptions.Compiled);

        /// <summary>
        ///     Retreives a texture from the Guild Wars 2 Render Service.
        /// </summary>
        /// <param name="uriOrSignatureFileIdPair">
        ///     Either the full Render Service URL or the signature and file id URI (e.g.
        ///     "7554DCAF5A1EA1BDF5297352A203AF2357BE2B5B/498983").
        /// </param>
        /// <param name="size">
        ///     Specifies the size of the texture requested - only some render service hosts will utilize this
        ///     setting.
        ...
        public AsyncTexture2D GetRenderServiceTexture(string uriOrSignatureFileIdPair, RenderServiceTextureSize size)
        {
            ...
            if (!splitUri.Success)
            {
                throw new ArgumentException(
                    $"Could not find signature / file id pair in provided '{uriOrSignatureFileIdPair}'.",
                    nameof(uriOrSignatureFileIdPair));
            }

            var signature = splitUri.Groups[1].Value;
            var fileId = splitUri.Groups[2].Value;
