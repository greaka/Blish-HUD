...
namespace Blish_HUD
{
    public class GameIntegrationService : GameService
    {
        ...
        {
            get => this._gw2HasFocus;
            private set
            {
                if (this._gw2HasFocus == value) return;

                this._gw2HasFocus = value;

                if (this._gw2HasFocus)
                {
        ...
        {
            get => this._gw2IsRunning;
            private set
            {
                if (this._gw2IsRunning == value) return;

                this._gw2IsRunning = value;

                if (this._gw2IsRunning)
                {
                    Gw2Started?.Invoke(this, EventArgs.Empty);
                }
                else
                {
                    Gw2Closed?.Invoke(this, EventArgs.Empty);
                }
            }
        }
        ...
        {
            get => this._gw2Process;
            set
            {
                if (this._gw2Process == value) return;

                this._gw2Process = value;

                if ((value == null) || (this._gw2Process.MainWindowHandle == IntPtr.Zero))
                {
                    BlishHud.Form.Invoke((MethodInvoker) (() => { BlishHud.Form.Visible = false; }));

                    this._gw2Process = null;
                }
                else
                {
                    this.Gw2WindowHandle = this._gw2Process.MainWindowHandle;

                    if (this._gw2Process.MainModule != null)
                    {
                        this._gw2ExecutablePath.Value = this._gw2Process.MainModule.FileName;
                    }
                }

                this.Gw2IsRunning = (this._gw2Process != null) &&
                                    (WindowUtil.GetClassNameOfWindow(this.Gw2Process.MainWindowHandle) ==
                                     GW2_GAMEWINDOW_NAME);
            }
        }

        public string Gw2ExecutablePath => this._gw2ExecutablePath.Value;
        ...
        {
            this._formWrapper = new Form();
            BlishHud.Form.Hide();
            BlishHud.Form.Show(this._formWrapper);
            BlishHud.Form.Visible = false;

            this._gameIntegrationSettings = Settings.RegisterRootSettingCollection(GAMEINTEGRATION_SETTINGS);

            DefineSettings(this._gameIntegrationSettings);
        }
        ...
        private void DefineSettings(SettingCollection settings)
        {
            ...
            this._gw2ExecutablePath = settings.DefineSetting("Gw2ExecutablePath", UNDEFINED_EXECPATH, "Gw2-64.exe Path",
                "The path to the game's executable. This is auto-detected, so don't change this unless you know what you're doing.");

            // We do this to avoid trying to detect in the registry
            // unless we have never detected the true path
            if (this._gw2ExecutablePath.Value == UNDEFINED_EXECPATH)
            {
                this._gw2ExecutablePath.Value = GetGw2PathFromRegistry();
            }
        }
        ...
        private string GetGw2PathFromRegistry()
        {
            try
            {
                using (var gw2Key = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64)
                    .OpenSubKey(GW2_REGISTRY_KEY, RegistryRights.ReadKey))
                {
                    if (gw2Key != null)
                    {
                        var gw2Path = gw2Key.GetValue(GW2_REGISTRY_PATH_SV).ToString();
            ...
            {
                Logger.Warn(ex,
                    "Failed to read Guild Wars 2 path from registry value {registryKey} located at {registryPath}.",
                    GW2_REGISTRY_PATH_SV, GW2_REGISTRY_KEY);
            }
        ...
        {
            BlishHud.Form.Shown += delegate { WindowUtil.SetupOverlay(BlishHud.FormHandle); };
        ...
        private void CreateTrayIcon()
        {
            ...
            this.ts_launchGw2Auto = this.TrayIconMenu.Items.Add(
                $"{Strings.GameServices.GameIntegrationService.TrayIcon_LaunchGuildWars2} - {Strings.GameServices.GameIntegrationService.TrayIcon_Autologin}");
            this.ts_launchGw2 =
                this.TrayIconMenu.Items.Add(Strings.GameServices.GameIntegrationService.TrayIcon_LaunchGuildWars2);

            this.ts_launchGw2Auto.Click += delegate { LaunchGw2(true); };
            this.ts_launchGw2.Click += delegate { LaunchGw2(); };
            ...
            this.ts_exit = this.TrayIconMenu.Items.Add($"{Strings.Common.Action_Exit} {Strings.Common.BlishHUD}");

            this.ts_exit.Click += delegate { this.ActiveBlishHud.Exit(); };

            this.TrayIconMenu.Opening += delegate
            {
                this.ts_launchGw2.Enabled = !this.Gw2IsRunning && File.Exists(this.Gw2ExecutablePath);
                this.ts_launchGw2Auto.Enabled = !this.Gw2IsRunning && File.Exists(this.Gw2ExecutablePath);
            };
        }
        ...
        private void TryAttachToGw2()
        {
            ...
            if (this.Gw2IsRunning)
            {
                ...
                catch (Win32Exception ex) /* [BLISHHUD-W] */
                {
                    ...
                    Logger.Warn(ex,
                        "A Win32Exception was encountered while trying to monitor the Gw2 process. It might be running with different permissions.");
                }
                ...
                BlishHud.Form.Invoke((MethodInvoker) (() => { BlishHud.Form.Visible = true; }));
            }
        }
        ...
        private Process GetGw2Process()
        {
            ...
            var gw2Processes = Process.GetProcessesByName(GW2_64_BIT_PROCESSNAME);
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            {
                this._lastGw2Check += gameTime.ElapsedGameTime.TotalSeconds;

                if (this._lastGw2Check > GW2_EXE_CHECKRATE)
                {
                    TryAttachToGw2();

                    this._lastGw2Check = 0;
                }
            }
        }
