...
namespace Blish_HUD.Modules.UI.Presenters
{
    ...
    {
        /// <inheritdoc />
        public ModuleDependencyPresenter(ModuleDependencyView view, ModuleDependencyCheckDetails[] model) : base(view,
            model)
        {
            /* NOOP */
        }
        ...
        {
            var dependencyRequirementsMet = true;
            ...
            foreach (var dependencyCheck in this.Model)
            {
                ...
                var dependencyVersion = dependencyCheck.Module == null
                ...
                switch (dependencyCheck.CheckResult)
                {
                    case ModuleDependencyCheckResult.NotFound:
                        dependencyMenuItem.StatusText = $"{ModulesService.Dependency_NotFound} v{dependencyVersion}";
                    ...
                    case ModuleDependencyCheckResult.AvailableNotEnabled:
                        dependencyMenuItem.StatusText = $"{ModulesService.Dependency_NotEnabled} v{dependencyVersion}";
                        dependencyMenuItem.StatusTextColor = Color.Yellow;
                        break;

                    case ModuleDependencyCheckResult.AvailableWrongVersion:
                        dependencyMenuItem.StatusText =
                            $"{ModulesService.Dependency_WrongVersion} v{dependencyVersion}";
                        dependencyMenuItem.StatusTextColor = Color.Yellow;
                        break;
