...
namespace Blish_HUD.Modules.Managers
{
    public class ContentsManager
    {
        ...
        private ContentsManager(IDataReader reader)
        {
            ...
            Logger.Info("New {contentsManagerName} instance utilizing a {dataReaderType} data reader.",
                nameof(ContentsManager), this._reader.GetType().FullName);
        }
        ...
        /// <summary>
        ///     Loads a <see cref="Texture2D" /> from a file such as a PNG. If the requested texture is inaccessible, the
        ///     <see cref="fallbackTexture" /> will be returned.
        /// </summary>
        /// <param name="texturePath">The path to the texture.</param>
        /// <param name="fallbackTexture">
        ///     An alternative <see cref="Texture2D" /> to return if the requested texture is not found
        ///     or is invalid.
        /// </param>
        public Texture2D GetTexture(string texturePath, Texture2D fallbackTexture)
        {
            using (var textureStream = this._reader.GetFileStream(texturePath))
            {
                if (textureStream != null)
                {
                    Logger.Info("Successfully loaded texture {texturePath} from {dataReaderFilePath}.", texturePath,
                        this._reader.GetPathRepresentation(texturePath));
                    return Texture2D.FromStream(GameService.Graphics.GraphicsDevice, textureStream);
                }
            }

            Logger.Warn("Unable to find texture {texturePath} at {dataReaderFilePath}.", texturePath,
                this._reader.GetPathRepresentation(texturePath));
        ...
        {
            long effectDataLength = this._reader.GetFileBytes(effectPath, out var effectData);
        ...
        {
            using (var soundStream = this._reader.GetFileStream(soundPath))
            {
                if (soundStream != null)
        ...
        {
            return this._reader.GetFileStream(filePath);
        }
    }
}