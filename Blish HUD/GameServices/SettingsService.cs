...
namespace Blish_HUD
{
    ...
    public class SettingsService : GameService
    {
        ...
        [JsonIgnore] private string _settingsPath;

        [JsonIgnore] internal JsonSerializerSettings JsonReaderSettings;

        [JsonIgnore] internal JsonSerializer SettingsReader;

        [JsonIgnore]
        public Dictionary<Type, SettingTypeRendererDelegate> SettingTypeRenderers =
            new Dictionary<Type, SettingTypeRendererDelegate>();
        ...
        protected override void Initialize()
        {
            ...
            this._settingsPath = Path.Combine(DirectoryUtil.BasePath, SETTINGS_FILENAME);

            // If settings aren't there, generate the file
            if (!File.Exists(this._settingsPath)) PrepareSettingsFirstTime();
        ...
        private void LoadSettings(bool alreadyFailed = false)
        {
            ...
            try
            {
                rawSettings = File.ReadAllText(this._settingsPath);

                this.Settings =
                    JsonConvert.DeserializeObject<SettingCollection>(rawSettings, this.JsonReaderSettings) ??
            ...
            catch (Exception ex)
            {
                ...
                {
                    Logger.Warn(ex,
                        "Failed to load settings due to an unexpected exception while attempting to read them. Already tried creating a new settings file, so we won't try again.");
                }
                else
                {
                    Logger.Warn(ex,
                        "Failed to load settings due to an unexpected exception while attempting to read them. A new settings file will be generated.");
        ...
        {
            if (!this.Loaded && !forceSave) return;

            var rawSettings = JsonConvert.SerializeObject(this.Settings, Formatting.Indented, this.JsonReaderSettings);

            try
            {
                using (var settingsWriter = new StreamWriter(this._settingsPath, false))
                {
        ...
        {
            this.SettingTypeRenderers.Clear();

            // bool setting renderer
            this.SettingTypeRenderers.Add(typeof(bool), (setting, panel) =>
            {
                ...
                Binding.CreateTwoWayBinding(() => settingCtrl.Checked,
                    () => strongSetting.Value);
            });
        }
        ...
        protected override void Load()
        {
            ...
            {
                Overlay.BlishHudWindow.AddTab(Strings.GameServices.SettingsService.SettingsTab,
                    Content.GetTexture("155052"), BuildSettingPanel(Overlay.BlishHudWindow), int.MaxValue - 1);
            };
        }

        internal SettingCollection RegisterRootSettingCollection(string collectionKey)
        {
            return this.Settings.DefineSetting(collectionKey, new SettingCollection()).Value;
        }
        ...
        public void RenderSettingsToPanel(Container panel, IEnumerable<SettingEntry> settings, int width = 325)
        {
            ...
            foreach (var settingEntry in listSettings)
            {
                ...
                var subTypeRenderer = this.SettingTypeRenderers
                    .FirstOrDefault(kv => settingEntry.SettingType.IsSubclassOf(kv.Key)).Value;
                ...
                Logger.Warn(
                    "Could not identify a setting renderer for setting {settingName} of type {settingType}, so it will not be displayed.",
                    settingEntry.EntryKey, settingEntry.SettingType.FullName);
            }
        }
        ...
        private Panel BuildSettingPanel(WindowBase wndw)
        {
            ...
            var settingsMiAbout = settingsListMenu.AddMenuItem(Strings.GameServices.OverlayService.AboutSection,
                Content.GetTexture("440023"));
            var settingsMiOverlay =
                settingsListMenu.AddMenuItem(Strings.GameServices.OverlayService.OverlaySettingsSection,
                    Content.GetTexture("156736"));
            var settingsMiModules = settingsListMenu.AddMenuItem(ModulesService.ManageModulesSection,
                Content.GetTexture("156764-noarrow"));

            settingsMiAbout.Click += delegate { cPanel.Show(new AboutView()); };

            settingsMiOverlay.Click += delegate
            {
                var overlaySettingsView =
                    new RepeatedView<IEnumerable<ApplicationSettingsPresenter.SettingsCategory>>();
                overlaySettingsView.Presenter = new ApplicationSettingsPresenter(overlaySettingsView,
                    ...
                    {
                        new ApplicationSettingsPresenter.SettingsCategory("General", Overlay._applicationSettings)
                    });

                cPanel.Show(overlaySettingsView);
            };

            Module.FinishedLoading += delegate
            {
                foreach (var module in Module.Modules)
                {
                    ...
                    moduleMi.Click += delegate { cPanel.Show(new ModuleManagementView(module)); };
                }
            };
