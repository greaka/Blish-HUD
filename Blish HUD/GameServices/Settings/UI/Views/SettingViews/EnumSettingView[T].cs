...
namespace Blish_HUD.Settings.UI.Views.SettingViews
{
    public class EnumSettingView<TEnum> : SettingView<TEnum> where TEnum : struct, Enum
    {
        ...
        /// <inheritdoc />
        public EnumSettingView(SettingEntry<TEnum> setting, int definedWidth = -1) : base(setting, definedWidth)
        {
            /* NOOP */
        }
        ...
        protected override Task<bool> Load(IProgress<string> progress)
        {
            ...
            this._enumValues = Enum.GetValues(typeof(TEnum)).Cast<TEnum>().ToArray();
        ...
        protected override void BuildSetting(Panel buildPanel)
        {
            ...
            this._enumDropdown = new Dropdown
            {
                Size = new Point(250, 27),
                Parent = buildPanel
            };

            this._enumDropdown.Items.AddRange(this._enumValues.Select(e => e.ToString()));

            this._enumDropdown.ValueChanged += EnumDropdownOnValueChanged;
        }
        ...
        private void EnumDropdownOnValueChanged(object sender, ValueChangedEventArgs e)
        {
            ...
            {
                this._enumDropdown.SelectedItem = this.Value.ToString();
            }
        }

        private void UpdateSizeAndLayout()
        {
            this.ViewTarget.Height = this._enumDropdown.Bottom;
            this._displayNameLabel.Height = this.ViewTarget.Height;

            if (this.DefinedWidth > 0)
            {
                this._enumDropdown.Left = this._displayNameLabel.Right + CONTROL_SIDEPADDING;

                this.ViewTarget.Width = this._enumDropdown.Right + CONTROL_SIDEPADDING;
            }
            else
            {
                this._enumDropdown.Location = new Point(this.ViewTarget.Width - CONTROL_SIDEPADDING - 250, 0);
            }
        }
        ...
        {
            this._enumDropdown.BasicTooltipText = description;
        }

        /// <inheritdoc />
        protected override void RefreshValue(TEnum value)
        {
            this._enumDropdown.SelectedItem = value.ToString();
        }
    }
}